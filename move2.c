...
/*   Updated: 2018/01/25 00:37:54 by yuxu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**test_swap_up(char **board, int line, int mv_up)
{
	char	*tmp;
	...
	{
		tmp = board[line];
		board[line] = board[line - mv_up];
		board[line - mv_up] = tmp;
	}
	return (board);
}
		...
		while (uni.i <= 3)
		{
			if (board[uni.l][uni.i] == '#')
			{
				...
				if (uni.hashtag_count == 1)
					uni.mv_up = uni.l;
				test_swap_up(board, uni.l, uni.mv_up);
			}
			uni.i++;
		}
		uni.l++;
